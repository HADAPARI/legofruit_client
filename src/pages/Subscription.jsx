  ...
  return (
    <div className="px-2 py-10 bg-gray-100 flex">
      ...
      <div className="py-24 sm:py-32 w-full">
          ...
          <div className="mt-8 grid grid-cols-4 gap-8">
            {cards.map((card, index) => (
              <SubscribeCards
                ...
                onClickAbonnement={() =>
                  handleAbonnementClick(card.buttonColor)
                }
              />
            ))}
          </div>
        ...
        {showPaymentModal && (
          <PaymentModal
            lastSelectedButtonColor={lastSelectedButtonColor}
            setPaymentButtonColor={setPaymentButtonColor}
          />
        )}{" "}
        {/* Passer la couleur du dernier bouton sélectionné et la fonction pour mettre à jour la couleur du bouton "Payer Maintenant" */}
      </div>
    </div>
  );
}
